...
public class SearchingWordList: MonoBehaviour
{
    ...
    private void CreateWordObjects() 
    {
        ...
        for (var index = 0; index < _wordsNumber; index++)
        {
            ...
            _words[index].GetComponent<SearchingWord>().SetWord(currentGameData.selectedBoardData.SearchingWord[index].Word);
        }
    }
...
{
   var startPositon = new Vector2(0f, transform.position.y);
   var squareRect = _words[0].GetComponent<RectTransform>();
   var parentRect = this.GetComponent<RectTransform>();
...
//Make sure they are in the centre
    ...
    startPositon.x = ((parentRect.rect.width - squareSize.x) / 2) * (-1);
    startPositon.x += shiftBy;
    startPositon.y = (parentRect.rect.height - squareSize.y) / 2;
    return startPositon;
}
